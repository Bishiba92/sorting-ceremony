...
function startQuiz(playerCount) {
    ...
    selectedAnswers = Array.from({ length: numPlayers }, () => []);
...
function nextQuestion() {
    ...
    currentPlayerAnswers.forEach((selectedOptionIndex, playerIndex) => {
        selectedAnswers[playerIndex][currentQuestionIndex] = selectedOptionIndex;
    });
...
function previousQuestion() {
    if (currentQuestionIndex > 0) {
        currentQuestionIndex--;
        currentPlayerAnswers = Array.from({ length: numPlayers }, (_, playerIndex) => selectedAnswers[playerIndex][currentQuestionIndex] || -1);
        displayQuestion();
    }
    scrollToTop();
}
...
function calculateScores() {
    // Reset scores
    score = Array.from({ length: numPlayers }, () => ({
        Gryffindor: 0,
        Ravenclaw: 0,
        Hufflepuff: 0,
        Slytherin: 0
    ...
    // Calculate scores based on saved answers
    selectedAnswers.forEach((answers, playerIndex) => {
        answers.forEach((selectedOptionIndex, questionIndex) => {
            if (selectedOptionIndex !== undefined && selectedOptionIndex !== -1) {
                ...
                // Add weights to each house score
                weights.forEach((houseWeight, index) => {
                    const house = houses[index];
                    score[playerIndex][house] += houseWeight;
                    totalWeight[playerIndex] += houseWeight;
                });
            }
        });
    });
